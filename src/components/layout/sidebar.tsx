...
export function Sidebar({
                                        ...
                                        <div key={item.title}>
                                            {hasChildren ? (
                                                <button
                                                    ...
                                                    className={`group flex w-full items-center justify-between rounded-[12px] px-3 py-2.5 text-xs font-medium transition-colors cursor-pointer ${isActive
                                                ...
                                                >
                                                    <div className="flex items-center gap-3">
                                                        <Icon
                                                            className={`h-4 w-4 shrink-0 ${isActive ? 'text-[#00BFFF]' : 'text-white/70'
                                                                }`}
                                                        />
                                                        {(!isCollapsed || isMobileOpen) && (
                                                ...
                                                <Link
                                                    ...
                                                    className={`flex items-center gap-3 rounded-[12px] px-3 py-2.5 text-xs font-medium transition-colors ${pathname === item.href
                                            ...
                                            {/* Submenu List */}
                                            {hasChildren && isOpen && (!isCollapsed || isMobileOpen) && (
                                                    ...
                                                    {item.children?.map((child) => {
                                                        ...
                                                        return (
                                                            <Link
                                                                ...
                                                                href={child.href}
                                                                onClick={() => isMobileOpen && onMobileClose()}
                                                                className={`flex items-center gap-2.5 rounded-[12px] px-2.5 py-2 text-xs transition-colors ${isChildActive
    ...
    return (
        <>
            ...
            <aside
                className={`hidden border-r border-[#0a2d6b] bg-[#001F5B] transition-all duration-300 lg:block ${isCollapsed ? 'w-16' : 'w-64'
                    }`}
            >
                {sidebarContent}
            </aside>
            ...
            {/* Mobile Drawer */}
            <aside
                className={`fixed inset-y-0 left-0 z-50 w-72 transform bg-[#001F5B] transition-transform duration-300 ease-in-out lg:hidden ${isMobileOpen ? 'translate-x-0' : '-translate-x-full'
                    }`}
            >
                {sidebarContent}
            </aside>
